...
if isnumeric(s2) & length(s2)==1    % Scalar
    ...
    if s1.type ~= s2.type,
        ...
        warning('XPARAM.MINUS: Arguments not of same type. Conversion performed'); 
    end
    
    if s1.reference ~= s2.reference,
        warning('XPARAM.MINUS: Arguments do not have the same reference impedence. Conversion NOT performed'); 
    end
